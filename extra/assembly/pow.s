! This program executes "pow" as a test program using the LC-5200 ISA
...
main:	lea $sp, initsp                           ! initialize the stack pointer
        ...
        beq $t0, $t1, TESTMIN                   ! test beq
        halt

TESTMIN:
        addi $t0, $zero, 1                      ! set $t0 for min check
        addi $t1, $zero, -1                     ! set $t1 for min check
        min $t2, $t0, $t1
        beq $t2, $t1, TESTMAX
        halt

TESTMAX:
        addi $t0, $zero, 1                      ! set $t0 for max check
        addi $t1, $zero, -1                     ! set $t1 for max check
        max $t2, $t0, $t1
        beq $t2, $t0, CALL
        halt
