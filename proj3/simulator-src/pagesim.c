...
FILE *read_args(int argc, char **argv)
{
    FILE *fin = 0;
    int opt;
    while (-1 != (opt = getopt(argc, argv, "i:htT:a:scr:")))
    {
        switch (opt)
        {
        ...
        case 't':
            ...
            exit(1);
        case 'T':
            if (run_select_test(optarg) < 0) {
                fprintf(stderr, "Unknown test %s.\n", optarg);
            }
            exit(1);
#ifdef AUTOGRADER
        case 'a':
            if (run_autograder_test(optarg) < 0) {
                fprintf(stderr, "Unknown autograder test %s.\n", optarg);
                exit(1);
            }
            exit(0);
#endif
...
void print_help_and_exit()
{
    ...
    printf("  -r\t\tSelect the replacement algorithm (either 'random' or 'lru' or 'clocksweep' for Second Chance)\n");
    ...
    printf("  -t\t\tRun tests\n");
    printf("  -T <testname>\t\tRun a specific test "
        "(e.g. get_vaddr_vpn, get_vaddr_offset, get_page_table, get_page_table_entry, get_physical_address, compute_stats, daemon_update, select_victim_frame_approx_lru, select_victim_frame_clocksweep). "
        "You must pass the address splitting test cases before using the other test cases.\n");
#ifdef AUTOGRADER
    printf("  -a <testname>\t\tRun a granular autograder test "
        "(e.g. system_init, free_frame, page_fault, mem_access, proc_init, proc_cleanup)\n");
#endif
